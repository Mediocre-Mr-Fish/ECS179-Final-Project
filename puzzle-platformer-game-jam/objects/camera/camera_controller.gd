...
func _camera_logic(_delta:float) -> void:
	...
	pushbox_position = _clamp_vec2_rect(
			pushbox_position,
			pushbox_top_left,
			pushbox_bot_right,
	)
	
	target_offset = _clamp_vec2_rect(
			target_offset,
			(pushbox_position - pushbox_size / 2) / 1,
			(pushbox_position + pushbox_size / 2) / 1,
	)
...
func _shifted_camera_logic(_delta:float) -> void:
	...
	if shift_tri_state.x < 0:
		target_offset -= shifted_offset_horizontal
	...
	if shift_tri_state.y < 0:
		target_offset += shifted_offset_up
		pushbox_position.y = pushbox_bot_right.y
	elif shift_tri_state.y > 0:
		target_offset += shifted_offset_down
		pushbox_position.y = pushbox_top_left.y
	else:
		pushbox_position.y = 0
	
	target_offset = target_offset / zoom_scale
	var target_position:Vector2 = subject.global_position + target_offset
	global_position += (target_position - global_position).limit_length(shift_speed * _delta)
